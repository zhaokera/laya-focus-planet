...
let Main = class Main extends Laya.Scene {
    ...
    drawBackground() {
        ...
        confetti.forEach(([x, y, c]) => {
            ...
            p.graphics.drawRect(0, 0, 10, 10, c);
            p.pos(x, y);
            p.alpha = 0.6;
    ...
    drawGridPreview() {
        ...
        for (let r = 0; r < rows; r++) {
            for (let c = 0; c < cols; c++) {
                ...
                if (!isColor)
                    cell.graphics.drawRect(0, 0, cw - 2, 4, "#EFE2C5");
                cell.pos(x, y);
                paper.addChild(cell);
    ...
    makeText(text, x, y, width, height, fontSize, color, bold) {
