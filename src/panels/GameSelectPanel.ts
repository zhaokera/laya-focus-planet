...
export type GameType = "schulte" | "memory";
...
export class GameSelectPanel extends Laya.Scene {
    private readonly BASE_W: number = 375;
    private readonly BASE_H: number = 750;

    private readonly GAMES: GameConfig[] = [
        {
            type: "schulte",
            name: "舒尔特方格",
            icon: "🔢",
            description: "按顺序点击数字，训练专注力",
            color: "#8B5CF6"
        },
        {
            type: "memory",
    ...
    private onSelectCallback: ((type: GameType) => void) | null = null;
     ...
     * 设置选择回调
     */
    public setOnSelect(callback: (type: GameType) => void): void {
    ...
    private createGameCards(): void {
        ...
        this.GAMES.forEach((game, index) => {
            const card = this.createGameCard(game, cardW, cardH);
            ...
            // 点击事件
            card.on(Event.CLICK, this, () => this.onGameSelect(game.type));
        });
    }

    private createGameCard(game: GameConfig, w: number, h: number): Laya.Sprite {
        ...
        for (let i = 0; i < steps; i++) {
            ...
            const color = `rgba(${this.hexToRgb(game.color)},${alpha})`;
            g.drawRect(x1, 0, stepW + 1, h, color);
        }

        // 边框
        g.drawRoundRect(0, 0, w, h, 12, null, `${game.color}40`, 1.5);

        // 图标背景圆
        const iconBgX = 20;
        const iconBgY = (h - 56) * 0.5;
        g.drawCircle(iconBgX + 28, iconBgY + 28, 28, `${game.color}30`);

        // 图标文字
        const iconText = new Laya.Text();
        iconText.text = game.icon;
    ...
    private onGameSelect(type: GameType): void {
        console.log("[GameSelect] selected:", type);
        ...
        // 跳转到对应游戏场景
        if (type === "schulte") {
            this.startSchulteGame();
        } else if (type === "memory") {
            this.startMemoryGame();
        }
    }

    private startSchulteGame(): void {
        // 跳转到舒尔特方格
        import("./GameScene").then((module) => {
            const scene = new module.GameScene();
            scene.name = "GameScene";
            scene.currentDifficulty = 5;
    ...
    private startMemoryGame(): void {
        // 跳转到记忆闪现
        import("./MemoryScene").then((module) => {
            const scene = new module.MemoryScene();
            scene.name = "MemoryScene";
            Laya.stage.addChild(scene);
            this.destroy();
        });
    }
