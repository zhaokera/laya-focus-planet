...
let GameScene = class GameScene extends Laya.Scene {
    constructor() {
        super(...arguments);
    ...
    onEnable() {
        var _a;
        this.currentDifficulty = this.normalizeDifficulty((_a = this.currentDifficulty) !== null && _a !== void 0 ? _a : this.currentDifficulty);
        this.buildScene();
    }
    setDifficulty(level) {
    ...
    makeText(text, x, y, width, height, fontSize, color, bold, strokeColor) {
    ...
    loadCoverImage(sp, assetPath, targetW, targetH) {
    ...
    loadSpriteImage(sp, assetPath, w, h) {
        const candidates = this.getAssetCandidates(assetPath);
        this.tryLoad(sp, candidates, 0, () => {
            sp.size(w, h);
        }, w, h);
    }
    tryLoad(sp, candidates, index, onLoaded, w, h) {
